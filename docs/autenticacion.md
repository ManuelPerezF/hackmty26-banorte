...
Implementado: registro y login reales con email/contraseña, Argon2id y sesiones revocables en PostgreSQL. Se eligieron sesiones opacas para este cliente web; JWT no es necesario para resolver el acceso ni la integración MCP.

## Credenciales y sesión

- Registro: nombre de 1–80 caracteres, email único normalizado y contraseña de 15–128 caracteres. La contraseña no se recorta ni normaliza.
...
Login/registro: máximo 20 solicitudes por IP y 10 por email normalizado por minuto. Rutas privadas: 180 por usuario/minuto. El limitador está en memoria y se reinicia con el proceso; no es un limitador distribuido.
...
Cada registro crea, dentro de una transacción, User, Profile, Account, dos tarjetas (Clásica y Oro) y nueve movimientos de ejemplo con UUID propios. Los siguientes movimientos manuales pertenecen a esa misma cuenta. Iniciar sesión no vuelve a sembrar datos.
